...
MEMORY_CAPACITY = 1000 # change to 100 000 (1 000 000 in original paper)
...
class Brain:
    ...
    def predict(self, states, target=False):
        """
            def predict(self, states, target=False):
                pred = np.array([BATCH_SIZE])
                for i in range (0,BATCH_SIZE):
                    print(states)
                    a = np.array([m.predict([states])[0] for m in self.models])
                    pred[i] = a
                print(pred)
                return pred
                """
        #for i in range (0,BATCH_SIZE-1):
        prediction = np.array([m.predict(states)[0] for m in self.model_])
        return prediction
    ...
    def updateTargetModel(self):
        self.model_.set_weights(self.model.get_weights()) # will not work
...
class Agent:
    ...
    def replay(self): # Update Tuples and Errors, than train the CNN   
        ...
        x, y, a, errors = self._getTargets(batch) # state, target, action, error
...
try:
    ...
    while True:
        ...
        #print(agent.exp, "/", MEMORY_CAPACITY)

        if save_counter % SAVE_XTH_GAME == 0: # all x games, save the CNN
            save_counter += 1

            print(" (Not True, but:) Saved model " + str(save_counter) + " to disk")
