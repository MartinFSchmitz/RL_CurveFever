...
class Policy_Brain:
    ...
    def train(self, state, target, action, action_prob, epoch=0, verbose=0):
    ...
    def predictOne(self, s, target=False):
        return self.predict(s.reshape(1, 2, 34+2, 34+2)).flatten()#8 0 = mapsize   try like this...
...
def reinforce(game, policy_brain, value_brain, num_episodes, discount_factor, pre):
    """
    ...
    for i_episode in range(num_episodes):
        ...
        for t, transition in enumerate(episode): # t is the counter, transition is one transition
            ...
            policy_brain.train(transition.state, advantage, transition.action, transition.action_prob) # do this for every transition (why?), but use total return
...
stats = reinforce(game, policy_brain, value_brain, MAX_EPISODES, GAMMA, pre)